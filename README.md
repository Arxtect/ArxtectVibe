...
* [x] **Mock数据模块 (mockDataBridge)**

  * [x] **模拟数据设计**：创建完整的用户、项目、文件数据结构模拟。包含多个示例用户账号（demo/demo, alice/alice, bob/bob），预置的LaTeX项目（包含多文件结构），以及协作者权限配置。确保数据结构与真实后端API完全一致。
  * [x] **接口实现**：实现与 dataBridge 完全相同的所有方法接口，包括登录认证、项目CRUD、文件操作、协作者管理等。所有方法都应包含合理的异步延迟模拟，提供真实的用户体验。
  * [x] **状态管理**：实现本地状态的增删改查，确保Mock模式下的数据操作能够持久保持（在刷新前），并且各个操作之间保持数据一致性。
  * [x] **错误模拟**：添加适当的错误模拟机制，如用户名密码错误、权限不足、资源不存在等场景，帮助测试前端的错误处理逻辑。

* [x] **首页模块 (Landing)**

  * [x] **模式切换功能**：在导航栏实现美观的toggle开关，支持在后端模式和Mock模式之间切换。切换时应清除当前用户状态，并在界面上给予明确的模式指示。
  * [x] **状态持久化**：确保模式选择能够保存到localStorage，用户刷新页面后仍能保持选择的模式。
  * [x] **模式提示**：在UI上清晰显示当前所处的模式（🌐 后端模式 / 🔧 Mock模式），并在各个操作后的toast提示中标明模式信息。

* [ ] **前端数据中间件 (dataBridge)**

  * [x] **智能模式切换**：实现根据 `isMockMode` 状态自动选择使用真实后端API或mockDataBridge的路由逻辑。确保所有方法调用都能根据当前模式正确分发到对应的实现。
  * [x] **状态管理增强**：扩展全局状态，添加 `isMockMode` 字段和 `setMockMode` 方法。切换模式时自动清除相关用户状态，避免数据混乱。
...
#### 1. 项目隔离问题
**现状**: 不同用户的项目没有实现有效隔离，用户可以访问到其他用户的项目数据。

**影响**:
- 数据安全风险：用户可能看到不属于自己的项目
- 多租户问题：Mock模式和真实模式下的项目数据可能混杂
- 权限控制失效：当前的权限检查机制不够严格

**涉及模块**:
- `src/core/dataBridge.ts` - 项目数据获取逻辑
- `src/core/mockDataBridge.ts` - Mock模式项目数据管理
- `src/modules/Editor/WebIDE/WebIDE.tsx` - 项目路径构造

**计划修复**:
- [ ] 在 DataBridge 层面实现用户级项目过滤
- [ ] 强化项目路径的用户隔离机制 
- [ ] 添加权限检查中间件，确保用户只能访问有权限的项目
- [ ] Mock模式下实现真正的多用户数据隔离

**优先级**: 🔴 高 - 涉及数据安全，需要优先解决

#### 2. WebIDE 插件系统类型不匹配
**现状**: 插件清单创建时存在类型定义不匹配问题。

**错误信息**:
```
Type '{ id: string; title: string; icon: string; }[]' is not assignable to type '{ [key: string]: ViewContainer[]; }'
```

**涉及文件**: `src/modules/Editor/WebIDE/WebIDE.tsx:153`

**计划修复**:
- [ ] 修复插件清单中 `viewContainers` 的类型定义
- [ ] 统一插件接口类型规范
- [ ] 完善插件类型验证机制

**优先级**: 🟡 中 - 影响开发体验，但不影响核心功能

### 📋 技术债务清单

#### 数据管理相关
- [ ] **真实后端Token验证**: 当前只有模拟实现，需要完整的JWT验证逻辑
- [ ] **状态持久化策略**: 统一Mock和真实模式的数据存储方式
- [ ] **错误处理标准化**: 建立统一的错误处理和用户反馈机制
- [ ] **数据同步优化**: 减少不必要的API调用和状态更新

#### WebIDE架构优化
- [ ] **插件动态加载**: 实现真正的插件懒加载和热重载
- [ ] **文件系统抽象**: 完善BrowserFS集成，支持更多存储后端
- [ ] **编辑器集成**: Monaco Editor和CodeMirror的深度集成
- [ ] **协同编辑**: Yjs文档同步机制的实现

#### 用户体验改进
- [ ] **加载状态优化**: 改进各模块的加载和错误状态显示
- [ ] **路由守卫增强**: 更精细的权限控制和页面访问管理
- [ ] **响应式设计**: 适配移动端和不同屏幕尺寸
...
- [ ] **资源预加载**: 关键资源的预加载策略
- [ ] **虚拟滚动**: 大文件列表的虚拟滚动实现
- [ ] **内存管理**: 防止内存泄漏，特别是在编辑器和协同会话中

#### 测试与质量保证
- [ ] **单元测试覆盖**: 核心模块的测试用例编写
- [ ] **集成测试**: 用户流程的端到端测试
- [ ] **性能测试**: 大项目和并发编辑的性能基准
- [ ] **类型安全**: 强化TypeScript类型检查，减少运行时错误
